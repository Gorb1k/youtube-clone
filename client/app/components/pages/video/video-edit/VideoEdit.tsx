...
import ToggleIsPublished from '../../../layout/header/upload-video/upload-video-form/toggle-isPublished/ToggleIsPublished'
...
const VideoEdit: FC = () => {
  ...
  const {
    ...
    control,
    handleSubmit,
  } = useForm<IVideoUpdate>({
    mode: 'onChange',
  })

  const { isLoading, data } = useQuery(
    ['get video for update', videoId],
    () => VideoService.getByVideoIdPrivate(videoId),
    {
      select: ({ data }) => data,
  ...
  const { mutate } = useMutation(
    ...
    (data: IVideoUpdate) => VideoService.update(videoId, data),
    {
      onSuccess: () => {
        router.push('/studio')
      },
